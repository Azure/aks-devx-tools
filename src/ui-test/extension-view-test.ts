...
describe('Extension View Test', () => {
    ...
    it('tests the extension info', async function () {
        ...
        devXToolsExtension = await extensions.findItem(`@installed ${packageJson.displayName}`) as ExtensionsViewItem;

        const desc = await devXToolsExtension.getDescription();
        const version = await devXToolsExtension.getVersion();

        assert.strictEqual(desc, 'Create deployment files and configure GitHub Actions workflows to deploy applications to Azure Kubernetes Service (AKS).');
