...
export function activate(context: vscode.ExtensionContext) {
  ...
  let disposableGenerateGitHubWorkflow = vscode.commands.registerCommand(
    "aks-draft-extension.runGenerateGitHubActionsWorkflow",
    async (folder) => {
      if (reporter) {
        reporter.sendTelemetryEvent("command", {
          command: "aks-draft-extension.runGenerateGitHubActionsWorkflow",
        });
      }
      // The code you place here will be executed every time your command is executed
      // Display a message box to the user
  ...
  context.subscriptions.push(disposableGenerateGitHubWorkflow);
